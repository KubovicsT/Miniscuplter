...
internal sealed class LauncherForm : Form
{
    ...
    public LauncherForm()
    {
        ...
        _runtime = new RuntimeSetupService(_settings);
    ...
    void BuildUi()
    {
        ...
        root.RowStyles.Add(new RowStyle(SizeType.AutoSize)); root.RowStyles.Add(new RowStyle(SizeType.AutoSize)); Controls.Add(root);
        ...
        info.Controls.Add(new Label { AutoSize = true, Text = "Install location: " + _settings.InstallRoot }, 1, 1); root.Controls.Add(info);
        ...
        _install.Click += async (_, _) => await ModelActionAsync("install"); _remove.Click += async (_, _) => await ModelActionAsync("remove");
        _updateModel.Click += async (_, _) => await ModelActionAsync("update"); _refresh.Click += async (_, _) => await RefreshAllAsync(initial: false);
        _repairRuntime.Click += async (_, _) => await RepairRuntimeAsync();
        ...
        buttons.Controls.AddRange(new Control[] { _install, _remove, _updateModel, _refresh, _repairRuntime, openModels, openInstall }); root.Controls.Add(buttons);
        ...
        _appUpdate.Click += async (_, _) => await ApplyApplicationUpdateAsync(); updateRow.Controls.Add(_status, 0, 0); updateRow.Controls.Add(_appUpdate, 1, 0); updateRow.Controls.Add(_progress, 2, 0); root.Controls.Add(updateRow);
    ...
    async Task RefreshAllAsync(bool initial)
    {
        ...
        try
        {
            ...
            {
                _hardware.Text = "AI runtime: unavailable"; _storage.Text = modelService.AvailabilityMessage;
                _grid.Rows.Clear(); _status.Text = "Local AI management needs its Python environment. Click Repair AI Runtime; the editor itself can still launch without AI.";
            }
            else
            {
                var snapshot = await modelService.GetStatusAsync(_settings.CheckModelUpdates);
                ...
                _status.Text = updates > 0 ? $"{updates} installed AI model update{(updates == 1 ? "" : "s")} available. Updates are never installed automatically."
            ...
            if (_settings.CheckApplicationUpdates)
            {
                try
                {
                    ...
                    _appVersion.Text = _latestApp.Available ? $"Application: v{_latestApp.CurrentVersion} · v{_latestApp.LatestVersion} available" : $"Application: v{_latestApp.CurrentVersion} · up to date";
                    _appUpdate.Enabled = _latestApp.Available && !string.IsNullOrWhiteSpace(_latestApp.DownloadUrl);
                    if (_latestApp.Available && string.IsNullOrWhiteSpace(_latestApp.DownloadUrl)) _status.Text += " The new release has no Windows ZIP update asset, so use its installer/release page.";
                }
    ...
    void PopulateModels(IEnumerable<ModelSnapshot> models)
    {
        ...
        {
            int index = _grid.Rows.Add(m.Id, m.Name, m.Kind, m.Installed ? "Yes" : "No", $"~{m.EstimatedGb:0.0} GB", ShortRevision(m.InstalledRevision), m.UpdateAvailable ? "AVAILABLE" : m.UpdateError != null ? "Check unavailable" : m.Installed ? "Current" : "—");
            var row = _grid.Rows[index]; row.Tag = m; if (m.UpdateAvailable) row.DefaultCellStyle.Font = new Font(_grid.Font, FontStyle.Bold); if (m.Id == selected) row.Selected = true;
        }
    ...
    async Task ModelActionAsync(string action)
    {
        ...
        try
        {
            ...
            if (action == "install") await service.InstallAsync(model.Id); else if (action == "remove") await service.RemoveAsync(model.Id); else await service.UpdateAsync(model.Id);
    ...
    {
        if (_busy) return;
        if (MessageBox.Show(this, "Repair/install the local AI Python environment now?\n\nThis installs AI runtime packages into the Miniscuplter installation. Model weights remain separate and are downloaded only when you choose Install for a model.", "Repair AI runtime", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes) return;
        SetBusy(true, "Preparing local AI runtime…");
        try { _status.Text = await _runtime.RepairAsync(); }
        catch (Exception ex) { MessageBox.Show(this, ex.Message, "AI runtime setup failed", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        finally { SetBusy(false); }
        await RefreshAllAsync(initial: false);
    }
    ...
    async Task ApplyApplicationUpdateAsync()
    {
        ...
        if (MessageBox.Show(this, $"Update Miniscuplter from v{info.CurrentVersion} to v{info.LatestVersion}?\n\nThe update is downloaded only after you approve it. AI models, projects, exports and user data are preserved.", "Application update", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes) return;
        SetBusy(true, $"Downloading Miniscuplter v{info.LatestVersion}…"); _progress.Visible = true; _progress.Value = 0;
        try
        {
            var progress = new Progress<int>(p => _progress.Value = Math.Clamp(p, 0, 100)); string package = await _updates.DownloadPackageAsync(info, progress);
            _status.Text = "Update downloaded. Restarting launcher into the staged updater…"; _updates.StartStagedUpdate(package); BeginInvoke(Close);
        }
    ...
    void LaunchApplication()
    {
        try
        {
            ...
            Process.Start(InstallLayout.CreateAppStartInfo(_settings)); WindowState = FormWindowState.Minimized;
        }
