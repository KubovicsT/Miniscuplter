...
public partial class Main
{
    ...
    public void InstallV01Extras()
    {
        ...
        if (aiPanel != null)
        {
            ...
            _aiPreview = new TextureRect { CustomMinimumSize = new Vector2(0, 220), ExpandMode = TextureRect.ExpandModeEnum.IgnoreSize, StretchMode = TextureRect.StretchModeEnum.KeepAspectCentered };
            aiPanel.AddChild(_aiPreview);
        }
        if (FindChild("ViewportHost", true, false) is SubViewportContainer host) host.GuiInput += OnRegionGuiInput;
    }

    void BeginRegionSelection()
    {
        _regionMode = true; _regionDragging = false;
    ...
    void OnRegionGuiInput(InputEvent ev)
    {
        ...
        {
            if (mb.Pressed) { _regionDragging = true; _regionStart = mb.Position; GetViewport().SetInputAsHandled(); }
            else if (_regionDragging)
            {
                _regionDragging = false; CreateRegionMask(_regionStart, mb.Position); _regionMode = false; GetViewport().SetInputAsHandled();
            }
        }
    }
    ...
    void CreateRegionMask(Vector2 a, Vector2 b)
    {
        ...
        var size = sub.GetVisibleRect().Size; int w = Math.Max(1, (int)size.X), h = Math.Max(1, (int)size.Y);
        int x0 = Math.Clamp((int)Math.Min(a.X, b.X), 0, w - 1), y0 = Math.Clamp((int)Math.Min(a.Y, b.Y), 0, h - 1);
    ...
    async Task AiEditSelectedRegion()
    {
        ...
        await RunAi(async () =>
        {
            ...
            ShowAiPreview(_lastEditedImage); SetStatus("Regional 2D edit generated. Approve it, regenerate, or generate the 3D part.");
        });
    }
    ...
    sealed class ObjectDto
    {
        ...
        public string Role { get; set; } = "mesh";
    ...
    void SaveProjectDialog()
    {
        ...
        AddChild(d); d.FileSelected += p => { SaveProject(p); d.QueueFree(); }; d.PopupCenteredRatio(.75f);
    }
    ...
    void LoadProjectDialog()
    {
        ...
        AddChild(d); d.FileSelected += p => { LoadProject(p); d.QueueFree(); }; d.PopupCenteredRatio(.75f);
    }
    ...
    void LoadProject(string projectPath)
    {
        try
        {
            ...
            string dir = Path.Combine(Path.GetDirectoryName(full)!, Path.GetFileNameWithoutExtension(full) + "_assets");
            foreach (var o in _objects) o.QueueFree(); _objects.Clear(); _selected = null; _undo.Clear(); _redo.Clear();
            foreach (var item in dto.Objects)
            {
                string meshPath = Path.Combine(dir, item.Mesh);
                if (!File.Exists(meshPath)) throw new FileNotFoundException("Project mesh asset is missing.", meshPath);
                AddMeshObject(MeshIO.LoadStl(meshPath), item.Name, new Vector3(item.Position[0], item.Position[1], item.Position[2]));
                if (_selected != null)
                {
