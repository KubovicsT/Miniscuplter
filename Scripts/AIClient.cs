...
public sealed class AIClient
{
    ...
    async Task<string> PostForFileAsync(string route, object payload)
    {
        ...
        var fullUrl = new Uri(new Uri(BackendUrl), route).ToString();
        using var response = await _http.PostAsync(fullUrl, new StringContent(json, Encoding.UTF8, "application/json"));
