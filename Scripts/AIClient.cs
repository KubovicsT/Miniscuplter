...
public sealed class AIClient
{
    ...
    public async Task<List<ReferenceResult>> SearchReferencesAsync(string query, int limit = 8)
    {
        ...
        foreach (var page in pages.EnumerateObject()) { var e = page.Value; string title = e.TryGetProperty("title", out var t) ? t.GetString() ?? "Reference" : "Reference"; string pageUrl = "https://commons.wikimedia.org/wiki/" + Uri.EscapeDataString(title.Replace(' ', '_')); string? thumb = null; if (e.TryGetProperty("imageinfo", out var ii) && ii.GetArrayLength() > 0) { var info = ii[0]; if (info.TryGetProperty("thumburl", out var tu)) { string? tuValue = tu.GetString(); if (!string.IsNullOrWhiteSpace(tuValue?.Trim())) thumb = tuValue.Trim(); else if (info.TryGetProperty("url", out var u)) { string? uValue = u.GetString(); if (!string.IsNullOrWhiteSpace(uValue?.Trim())) thumb = uValue.Trim(); } } else if (info.TryGetProperty("url", out var u)) { string? uValue = u.GetString(); if (!string.IsNullOrWhiteSpace(uValue?.Trim())) thumb = uValue.Trim(); } } results.Add(new ReferenceResult(title, pageUrl, thumb)); }
        return results;
    }
}
