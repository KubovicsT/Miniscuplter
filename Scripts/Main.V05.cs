...
public partial class Main
{
    ...
    public void InstallV05Extras()
    {
        ...
        _v05Quality.AddItem("Preview");
        _v05Quality.AddItem("Standard");
        _v05Quality.AddItem("High");
        ...
        var erase = new Button { Text = "Erase Mask" };
        var clear = new Button { Text = "Clear" };
        paint.Pressed += () => BeginPaintMask(false);
        erase.Pressed += () => BeginPaintMask(true);
        clear.Pressed += ClearPaintMask;
        ...
        ai.AddChild(new Label { Text = "Mask brush radius" });
        ai.AddChild(radius);

        var candidates = new Button { Text = "Generate 4 Preview Candidates" };
        candidates.Pressed += async () => await GenerateCandidateSet();
        ai.AddChild(candidates);
        _v05Candidates = new VBoxContainer();
        ai.AddChild(_v05Candidates);

        var patch = new Button { Text = "Generate 3D Patch at Mask Anchor" };
        patch.Pressed += async () => await GenerateAlignedPatch();
        ai.AddChild(patch);

        var history = new Button { Text = "Reset Learned Time Estimates" };
        history.Pressed += ResetJobHistory;
        ai.AddChild(history);

        if (FindChild("ViewportHost", true, false) is SubViewportContainer host)
            host.GuiInput += OnV05ViewportInput;
    ...
    void BeginPaintMask(bool erase)
    {
        ...
        if (sub == null) return;
        int w = Math.Max(1, (int)sub.GetVisibleRect().Size.X);
        int h = Math.Max(1, (int)sub.GetVisibleRect().Size.Y);
        if (_v05PaintMask == null || _v05PaintMask.GetWidth() != w || _v05PaintMask.GetHeight() != h)
        {
            _v05PaintMask = Image.Create(w, h, false, Image.Format.L8);
            _v05PaintMask.Fill(Colors.Black);
        }
        _v05MaskErase = erase;
        _v05MaskPaintMode = true;
    ...
    void OnV05ViewportInput(InputEvent ev)
    {
        ...
        {
            var ro = _camera.ProjectRayOrigin(p.Value);
            var rd = _camera.ProjectRayNormal(p.Value);
            if (RayMesh(ro, rd, _selected, out var hit)) _v05Anchor = hit;
        }
    ...
    void PaintMaskCircle(int cx, int cy, float value)
    {
        ...
        int r = _v05MaskRadius;
        int w = _v05PaintMask.GetWidth(), h = _v05PaintMask.GetHeight();
    ...
    {
        if (_v05PaintMask == null) return;
        string dir = ProjectSettings.GlobalizePath("user://masks");
        ...
        SetStatus("Painted AI mask saved.");
    }
    ...
    async Task GenerateCandidateSet()
    {
        ...
        {
            int n=i+1;
            string outPath = ProjectSettings.GlobalizePath($"user://candidate_{DateTime.Now:yyyyMMdd_HHmmss}_{n}.png");
            var sw = Stopwatch.StartNew();
            string result = await _ai.EditImageAsync(source, string.IsNullOrEmpty(_v05PaintMaskPath)?null:_v05PaintMaskPath, prompt, outPath, "preview");
        ...
        RefreshEtaLabel();
        SetStatus("Generated 4 preview candidates. Choose one before creating the 3D patch.");
    }
    ...
    async Task GenerateAlignedPatch()
    {
        ...
        string outPath = ProjectSettings.GlobalizePath($"user://ai_patch_{DateTime.Now:yyyyMMdd_HHmmss}.stl");
        string prompt = _prompt?.Text.Trim() ?? "";
        var sw = Stopwatch.StartNew();
        await RunAi(async () =>
        {
            ...
            if (_selected != null && _v05Anchor is Vector3 anchor)
                _selected.Position = anchor;
        });
        sw.Stop(); RecordJob("3d", quality, sw.Elapsed.TotalSeconds);
        RefreshEtaLabel();
        SetStatus($"3D patch added as a separate AI edit layer in {FormatSeconds(sw.Elapsed.TotalSeconds)}. Fine-tune transform, then voxel-union when accepted.");
    }
    ...
    void RecordJob(string type, string quality, double seconds)
    {
        ...
        while (_v05History.Count > 200) _v05History.RemoveAt(0);
        SaveJobHistory();
    }
    ...
    string EstimateText(string type, string quality)
    {
        ...
        double mean=weighted/weights;
        double min=samples.Min(x=>x.Seconds), max=samples.Max(x=>x.Seconds);
    ...
    {
        if (_v05Eta == null) return;
        string q=CurrentQuality();
    ...
    void LoadJobHistory()
    {
        try
        {
            ...
            var list=JsonSerializer.Deserialize<List<JobSample>>(File.ReadAllText(HistoryPath()));
            if (list!=null) _v05History.AddRange(list);
        }
        catch { }
    }
