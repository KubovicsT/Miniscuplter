...
public partial class Main
{
    ...
    public void InstallV03Extras()
    {
        ...
        aiPanel.AddChild(new Label { Text = "Use AI to generate a concept, or load your own artwork/reference image and continue from there.", AutowrapMode = TextServer.AutowrapMode.WordSmart });
        var row = new HBoxContainer();
        var load = new Button { Text = "Use My Image", SizeFlagsHorizontal = Control.SizeFlags.ExpandFill };
        var clear = new Button { Text = "Clear" }; load.Pressed += OpenStartingImageDialog; clear.Pressed += ClearStartingImage;
        row.AddChild(load); row.AddChild(clear); aiPanel.AddChild(row);
        var edit = new Button { Text = "Edit Starting Image With AI" };
        edit.Pressed += async () => await EditStartingImageWithAi(); aiPanel.AddChild(edit);
        _v03SourceStatus = new Label { Text = "Starting source: AI-generated concept or viewport capture", AutowrapMode = TextServer.AutowrapMode.WordSmart };
    ...
    void OpenStartingImageDialog()
    {
        var d = new FileDialog
        {
            ...
            Filters = new[] { "*.png ; PNG images", "*.jpg,*.jpeg ; JPEG images", "*.webp ; WebP images", "*.bmp ; BMP images" },
            UseNativeDialog = true
        };
        AddChild(d);
        d.FileSelected += path => { try { SetStartingImage(path); } finally { d.QueueFree(); } };
    ...
    void SetStartingImage(string sourcePath)
    {
        try
        {
            ...
            int originalW = image.GetWidth(), originalH = image.GetHeight();
            if (image.DetectAlpha() != Image.AlphaMode.None)
            {
                var flattened = Image.CreateEmpty(originalW, originalH, false, Image.Format.Rgb8);
                Color bg = new(0.5f, 0.5f, 0.5f);
                for (int y = 0; y < originalH; y++) for (int x = 0; x < originalW; x++)
                {
                    Color c = image.GetPixel(x, y); float a = c.A;
                    flattened.SetPixel(x, y, new Color(c.R * a + bg.R * (1-a), c.G * a + bg.G * (1-a), c.B * a + bg.B * (1-a)));
                }
                image = flattened;
            }
            else image.Convert(Image.Format.Rgb8);
            ...
            if (_v03SourceStatus != null)
                _v03SourceStatus.Text = $"Starting source: user image · {Path.GetFileName(sourcePath)} · normalized {image.GetWidth()}×{image.GetHeight()}";
            SetStatus("User image loaded and normalized. Transparency was flattened safely and oversized inputs are capped at 2048 px.");
        }
        catch (Exception ex) { SetStatus("Could not load starting image: " + ex.Message); }
    }
    ...
    async Task EditStartingImageWithAi()
    {
        ...
        await RunAi(async () =>
        {
            ...
            if (_v03SourceStatus != null) _v03SourceStatus.Text = "Starting source: user image · AI-edited";
    ...
    {
        string old = _v03StartingImage; _v03StartingImage = "";
        if (_lastEditedImage == old) _lastEditedImage = "";
        if (_v03SourceStatus != null) _v03SourceStatus.Text = "Starting source: AI-generated concept or viewport capture";
