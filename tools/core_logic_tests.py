...
def test_stage_c_ui_wiring():
    ...
    acceptance=(ROOT/"Scripts/Main.V1022Acceptance.cs").read_text(encoding="utf-8")
    stagec=(ROOT/"Scripts/Main.V1020StageCBridge.cs").read_text(encoding="utf-8")
    restore=(ROOT/"Scripts/Main.V1020StageCRestore.cs").read_text(encoding="utf-8")
    check("InstallV1022Acceptance();" in installer,"v1.0.22 acceptance installer missing")
    check(installer.index("InstallV1020StageCEditingAuthority();") < installer.index("InstallV1022Acceptance();"),"v1.0.22 acceptance guard must run after Stage-C composition")
    check("Pressed -= V1017Generate3DAsync" in acceptance and "Pressed -= V109Generate3DAsync" in acceptance,"legacy Generate-3D owners are not explicitly removed")
    check("Pressed -= V1020Generate3DAsync" in acceptance and "Pressed += V1020Generate3DAsync" in acceptance,"Stage-C Generate-3D owner is not rebound exactly once")
    check("V1020Generate3DAsync" in stagec and "RegisterResult" in stagec and "V1020ApplyCandidate" in stagec,"production generation does not preserve candidate/apply semantics")
    check("CandidateStatus.Applied" in restore and "candidate.OutputObjectId" in restore and "V1020ProjectObjectStateToScene" in restore,"applied-object restore is not identity/revision bound")
    ...
    check("V1022ReassertClientFill" in acceptance and "SyncV109RootToViewport();" in acceptance and "root.SetAnchorsAndOffsetsPreset" not in acceptance,"whole-window client fill guard does not delegate to the V109 root owner")
...
def test_v1023_ui_preferences_wiring():
    installer=(ROOT/"Scripts/ExtrasInstaller.cs").read_text(encoding="utf-8")
    prefs=(ROOT/"Scripts/Main.V1023UiPreferences.cs").read_text(encoding="utf-8")
    check("InstallV1023UiPreferences();" in installer,"v1.0.23 UI preference installer missing")
    check(installer.index("InstallV1022Acceptance();") < installer.index("InstallV1023UiPreferences();"),"UI preference layer must observe the final accepted workspace composition")
    check('AppDataRoot.Resolve("Settings/ui_preferences.json")' in prefs,"UI preferences are not stored under the authoritative data root")
    check("BodySplitOffset" in prefs and "WorkspaceSplitOffset" in prefs,"workspace splitter persistence is incomplete")
    check("V1023ReflowWorkspaceSplit" in prefs and "V1023ClampSplit" in prefs and "int maximumPrimary = Math.Max" in prefs,"workspace preference owner does not safely reflow narrow split bounds")
    check("FontScale" in prefs and "0.75" in prefs and "1.35" in prefs,"bounded UI/font scaling contract missing")
    check("V1023AttachInterfaceSettings" in prefs and "V1023InstallTooltips" in prefs,"interface settings/tooltip infrastructure missing")
    check("File.Move(temp, path, true)" in prefs,"UI preference writes are not replacement-safe")
    check("ProjectStore" not in prefs and "V1020Generate3DAsync" not in prefs,"UI preferences must not become project or AI action authority")
if __name__=="__main__":test_quality_clamps();test_model_routing();test_readiness_aware_routing();test_provider_readiness_contract();test_capabilities();test_modern_image_low_vram_strategy();test_component_file_validation();test_storage_path_boundaries();test_uninstall_path_guard();test_resumable_stage_recovery();test_stage_c_ui_wiring();test_v1023_ui_preferences_wiring();__import__("runpy").run_path(str(ROOT/"tools/localdev_tests/test_ld_20260920_1006.py"),run_name="__main__");print("v1.0.23 core logic, Stage-C wiring and UI preference tests passed")
