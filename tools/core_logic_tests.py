...
def test_model_routing():
    oi,oh,or_=model_router.installed,model_router.hardware_info,model_router.check_3d_provider
    class Ready:
        def __init__(self,provider,ready=True,error=""):
            self.provider=provider;self.ready=ready;self.errors=(() if ready else (error or "not ready",))
    try:
        present={"sdxl-base","sd21","triposr","hunyuan21-shape","partcrafter","hunyuan2mini","sf3d"};model_router.installed=lambda x:x in present;model_router.hardware_info=lambda:{"vram_mb":8192};model_router.check_3d_provider=lambda p:Ready(p,True)
        ...
        # An installed provider that fails readiness must be skipped before a long generation starts.
        model_router.check_3d_provider=lambda p:Ready(p,p!="sf3d","missing dependency")
        check(model_router.choose_3d_provider("fast").provider=="triposr","unready preferred provider should fall back before inference")
        try:model_router.choose_3d_provider("fast","sf3d");raise AssertionError("explicit unready provider accepted")
        except RuntimeError as exc:check("not ready" in str(exc),"explicit readiness failure not explained")
        present.update({"flux2-klein-4b","qwen-image-edit","qwen-image-2512","z-image-turbo","spar3d","partpacker","trellis2"});model_router.hardware_info=lambda:{"vram_mb":24576};model_router.check_3d_provider=lambda p:Ready(p,True);check(model_router.choose_image_provider("generate").provider=="qwen","24GB concept should prefer Qwen");check(model_router.choose_image_provider("detail").provider=="qwen-edit","24GB edit should prefer Qwen Edit");check(model_router.choose_3d_provider("quality").provider=="trellis2","24GB quality should prefer TRELLIS.2");check(model_router.choose_3d_provider("structured").provider=="partpacker","24GB parts should prefer PartPacker")
    ...
    finally:model_router.installed,model_router.hardware_info,model_router.check_3d_provider=oi,oh,or_
