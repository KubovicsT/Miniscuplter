...
def test_stl_topology_is_welded_for_analysis() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        with isolated_data(tmp):
            ...
            check(report["nonmanifold_edges"] == 0, f"closed cube reported non-manifold edges: {report['nonmanifold_edges']}")
...
def test_voxel_remesh_round_trip() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        with isolated_data(tmp) as data:
            source = Path(tmp) / "source.stl"
            output = data / "remeshed.stl"
...
def test_output_path_stays_contained() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        with isolated_data(tmp) as data:
            source = data / "source.stl"
            trimesh.creation.box().export(source, file_type="stl")
            outside = Path(tmp).parent / "outside.stl"
            try:
                geometry_ops.voxel_remesh([str(source)], str(outside), voxel_size=1.0)
                raise AssertionError("geometry output escaped the configured data root")
            except ValueError:
                pass
...
if __name__ == "__main__":
    ...
    test_output_path_stays_contained()
    print("v1.0.19 geometry safety tests passed")
