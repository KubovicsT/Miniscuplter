...
def _limit_input(image: Image.Image) -> Image.Image:
    ...
    if max(image.size) <= max_px: return image
    work = image.copy(); work.thumbnail((max_px, max_px), Image.Resampling.LANCZOS); return work
...
def _torch_and_model():
    try:
        ...
        raise RuntimeError("Local 2D AI dependencies are not installed. Use Miniscuplter Launcher -> Repair AI Runtime, then install Stable Diffusion 2.1 if you want the legacy fallback.") from exc
    model = component_path("sd21")
    if model is None: raise RuntimeError("Stable Diffusion 2.1 is not installed. Install it from Miniscuplter Launcher or the AI Models panel.")
...
def _img_pipe():
    global _IMG_PIPE
    if _IMG_PIPE is not None: return _IMG_PIPE
...
def generate_concept(prompt: str, output_path: str, quality: str = "standard") -> str:
    cfg = _q(quality); pipe = _text_pipe()
    result = pipe(prompt=prompt, negative_prompt="blurry, low detail, text, watermark, cropped, malformed anatomy",
                  num_inference_steps=cfg["steps"], guidance_scale=cfg["guidance"], width=cfg["size"], height=cfg["size"]).images[0]
...
def edit_image(image_path: str, mask_path: Optional[str], prompt: str, output_path: str, quality: str = "standard") -> str:
    cfg = _q(quality); pipe = _img_pipe(); original = _limit_input(Image.open(image_path).convert("RGB")); work = original.resize((cfg["size"], cfg["size"]), Image.Resampling.LANCZOS)
    result = pipe(prompt=prompt, negative_prompt="blurry, low detail, text, watermark, malformed anatomy", image=work,
                  strength=cfg["strength"], guidance_scale=cfg["guidance"], num_inference_steps=cfg["steps"]).images[0]
    if mask_path and Path(mask_path).exists():
        mask = Image.open(mask_path).convert("L").resize(original.size, Image.Resampling.BILINEAR).filter(ImageFilter.GaussianBlur(radius=4))
        final = Image.composite(result.resize(original.size, Image.Resampling.LANCZOS), original, mask)
    else: final = result.resize(original.size, Image.Resampling.LANCZOS)
    out = Path(output_path); out.parent.mkdir(parents=True, exist_ok=True); final.save(out); return str(out)


def release_models() -> None:
    global _PIPE, _IMG_PIPE
    _PIPE = None; _IMG_PIPE = None
    try:
        import torch
