...
COMPONENTS: dict[str, dict[str, Any]] = {
    "sd21": {
        ...
        "description": "Default local 2D concept and inpainting model for v0.2.",
        "estimated_gb": 5.5,
    },
    "hunyuan21-shape": {
        "name": "Hunyuan3D 2.1 Shape",
        "kind": "3d",
        "source": "hunyuan",
        "repo_id": "tencent/Hunyuan3D-2.1",
        "code_url": "https://github.com/Tencent-Hunyuan/Hunyuan3D-2.1.git",
        "description": "Official Tencent image-to-shape model. Texture generation is intentionally omitted for miniature/STL use.",
        "estimated_gb": 10.0,
    },
}
...
def hardware_info() -> dict[str, Any]:
    ...
    if nvidia_smi:
        try:
            ...
            info["gpu"] = name
            info["vram_mb"] = int(mem)
            info["cuda_available"] = True
            if info["vram_mb"] >= 12000:
                info["recommended_profile"] = "quality"
            elif info["vram_mb"] >= 8000:
                info["recommended_profile"] = "low-vram"
            else:
                info["recommended_profile"] = "cpu-offload"
        except Exception:
            pass
    return info
...
def status() -> dict[str, Any]:
    ...
    for component_id, spec in COMPONENTS.items():
        ...
        result.append(entry)
    return {"hardware": hardware_info(), "components": result, "data_root": str(DATA_ROOT)}
...
def install_component(component_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError("huggingface_hub is required. Run setup_ai_backend.bat again for v0.2 dependencies.") from exc
    ...
    if component_id == "sd21":
        ...
        if not code_dir.exists():
            ...
            if not git:
                raise RuntimeError("Git is required to install the official Hunyuan3D source code.")
...
def uninstall_component(component_id: str) -> dict[str, Any]:
    state = load_state()
    entry = state.get("installed", {}).get(component_id)
    if entry and entry.get("path"):
        path = Path(entry["path"])
        if path.exists() and DATA_ROOT in path.parents:
...
def component_path(component_id: str) -> Path | None:
    state = load_state()
    entry = state.get("installed", {}).get(component_id)
    if not entry or not entry.get("installed") or not entry.get("path"):
        return None
    p = Path(entry["path"])
