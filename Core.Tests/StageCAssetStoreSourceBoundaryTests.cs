    ...
    internal static void ValidateSourceBoundaryAndCopy()
    {
        ...
        try
        {
            foreach (string? invalid in new string?[] { null, "", "   " })
            {
                try
                {
                    StageCAssetStore.CreateImageRevisionAsync(projectPath, invalid!, "baseline", "ld-1005")
            ...
            ImageRevision revision = StageCAssetStore.CreateImageRevisionAsync(
                projectPath, source, " accepted-purpose ", " test-provenance ").GetAwaiter().GetResult();
            ...
            Require(File.ReadAllBytes(durable).SequenceEqual(payload), "copied payload changed");
