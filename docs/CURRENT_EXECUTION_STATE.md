...
- next:
  1. treat the symmetric missing-input/output candidate validation coverage as acceptance hardening, not by itself as a release-sized runtime-test batch; take the next related slice from terminal/persistence behavior only if it adds or proves meaningful state behavior beyond test symmetry
  2. otherwise advance the related stale presentation migration paths for protected-region/selection/candidate state after Core revision authority changes or stable object identity can no longer be proven; stale state must clear/fail closed rather than remain visually authoritative
  ...
  4. CI-latency-aware continuation is authorized: after a slice has focused/Core validation green, Dev may continue with another closely related independent reversible slice while full build/package is still pending when no release/freeze/no-race boundary exists and the next slice does not depend on the pending result; preserve exact-head attribution and reconcile any failure before dependent work or release return
  5. before returning a release checkpoint, obtain required final exact-head Core plus full build/package green for the complete coherent candidate; once 2–4 related meaningful changes form a useful runtime-test batch, return v1.0.35 to Coordinator for release and roll remaining adjacent work to v1.0.36
