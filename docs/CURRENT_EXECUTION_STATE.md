...
## Current objective
### G — v1.0.35 coherent batch ready for Coordinator release review
- state: READY FOR COORDINATOR RELEASE REVIEW
- outcome: bounded revision-safety/presentation-authority batch is assembled and exact-head green; defer adjacent foundation work to the next semantic version unless Coordinator rejects the release boundary
...
- refinement_candidate_persistence_contract: when one Ready Refinement candidate advances an object away from a shared input revision, competing sibling candidates retain their exact input/output dependency identity as durable Conflict records across save/reopen; a valid persisted sibling Conflict reopens clean without a spurious reconciliation repair or dirty state; rejected Apply after reopen remains fail-closed and cannot mutate the applied output state; explicit discard of a reopened sibling Conflict is transactional, preserves the already-applied output revision, clears the stale diagnostic, round-trips through undo/redo, and persists/reopens as clean Discarded state. Missing input and output revision identity are both explicitly rejected by ProjectState validation before a candidate can enter durable state. Exact-head Core and full build/package validation are green.
...
- selection_dependency_contract:
  ...
  11. when stable selection changes to a different Core-mapped object, prior-object Smart Selection presentation is cleared before reconciliation; only a current durable binding matching the newly selected ObjectId/revision may restore weights
- next:
  1. Coordinator performs release-boundary review for this exact v1.0.35 checkpoint; Dev does not broaden v1.0.35 while that review is pending
  2. if Coordinator accepts the batch, Coordinator owns publication and validated forward-version bootstrap; remaining adjacent stale-presentation/attachment migration work rolls to the next semantic version
  3. if Coordinator rejects the release boundary for a concrete acceptance gap, Dev resumes only that bounded gap on v1.0.35 and restores final exact-head Core + full build/package green before re-handoff
...
- continuation: release-review boundary reached for v1.0.35; hold adjacent product scope on this semantic branch until Coordinator accepts or rejects the checkpoint. Independent user verification of v1.0.34 remains non-blocking.
