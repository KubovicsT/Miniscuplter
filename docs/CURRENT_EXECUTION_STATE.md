...
- current: revision-safe Refinement candidate/dependency hardening now rejects invalid output lineage immediately instead of leaving incompatible candidates Ready until Apply; continue UI-neutral dependency preservation/invalidation and migration hardening
- completed_this_slice: Core attachments bind exact parent/child mesh revisions, become stale/non-authoritative when either revision advances, support explicit transactional rebind, reject stale updates, preserve exact state through save/reopen and undo/redo, and now drive mapped Kitbash attachment actions through stable ObjectId/AttachmentRecord authority. The existing Kitbash UI remains unchanged; its legacy DTO is a presentation/compatibility projection for mapped objects rather than durable placement authority. Mapped stale attachments no longer continue driving placement from display-name DTO state. Unmigrated legacy objects retain the existing fallback until they acquire stable Core object identity. Smart Selection grow/shrink/smooth now reconciles the changed weight array into the existing durable revision-bound selection persistence path instead of leaving refined weights only in Godot memory. Ready Refinement candidates are now conflicted as soon as their output revision is not descended from their bound input revision, rather than remaining apparently applicable until a later Apply attempt. Exact-head Core, C#, Python/runtime, release-audit and full packaging validation for the implementation changes are green.
...
- process_blocker: none; recurring patch-request formatting friction was fail-closed and recovered within the bounded retry contract; AUTO-INC-013 carries the process evidence
