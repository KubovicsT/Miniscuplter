...
- refinement_candidate_persistence_contract: when one Ready Refinement candidate advances an object away from a shared input revision, competing sibling candidates retain their exact input/output dependency identity as durable Conflict records across save/reopen; rejected Apply after reopen remains fail-closed and cannot mutate the applied output state. Exact-head Core and full build/package validation are green.
