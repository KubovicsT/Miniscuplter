...
- **Attempts/fixes:** v1.0.12–v1.0.19 hardened provider, viewport, storage, geometry and update seams. v1.0.20 commits through `8d394f289e02682bf27a133ed3456c7dc2852df9` added the tested Core `StageCGeneration` contract. Later v1.0.20 work through `cfd0352223cada181ea75843f16b25b9b5ceb541` made accepted images project-owned immutable revisions, rewired the production baseline/3D callbacks onto `ProjectSession`, made generated STL a durable Ready/Conflict candidate requiring explicit Apply/Discard, and restored applied objects visibly after restart. Commits `ebfa4fce7691e71cbfaffc16832e171dd83bd839`, `1429d9bc4c45db2cbca0a9b99b3bf2bb6f93d674`, `ba99de7e59303c33805565df5159d2fa83a89bc1`, `4885f80d825213d66af957397a03045a80432328`, and `a9f7e0989d5486bdee27f054edb58713aa00cc41` then carried the full generation identity through AIClient/backend job context and fail-closed response verification.
- **Result:** the production baseline→3D candidate seam now has durable Core state plus end-to-end transport identity. The whole cleanup/export slice and real GTX1080 qualification remain incomplete.
- **Verification:** `core-foundation` run `34493357060` passed for `a9f7e098...`; broader build `34493356971` passed Python/core/job/geometry/release-audit and C# build jobs, with portable package/layout/SHA also passed at the last check. Final packaging conclusion must still be reconciled before release.
- **Next action:** harden save-failure recovery, continue cleanup/export integration, then run the complete slice on the target machine.
...
- **Attempts/fixes:** cancellation reset in v1.0.13; job progress in v1.0.17/v1.0.18; v1.0.20 `StageCGeneration` added stable `GenerationJobId`/binding carrying project identity, project revision, exact input ImageRevision and reserved output ObjectId. Production editor integration then bound result registration to that Core contract. This run completed transport correlation for the Stage-C 3D seam: `AIClient.Generate3DStageCAsync()` uses the Core GenerationJobId as the transport job ID and sends ProjectId/project revision/ImageRevisionId/ObjectId; backend rejects partial or header-mismatched identity, stores the complete context in job progress, and echoes it in the verified response; AIClient rejects missing/mismatched echoed context before candidate registration. Job context snapshots are deep-copied and regression-tested.
- **Result:** stale/replayed/cross-project transport results can no longer silently proceed through the migrated Stage-C 3D candidate path solely because an output file exists. This substantially closes the first vertical-slice correlation gap without claiming the final durable Job Broker is finished.
- **Verification:** C# compilation validates the typed AIClient/editor integration; Python job-progress tests cover retained/isolated context; `core-foundation` run `34493357060` passed and broader build `34493356971` passed its C# and Python/core/job/geometry jobs at the last check.
- **Next action:** harden ProjectStore save-failure consistency, then continue toward durable queue/resource ownership/crash recovery after the thin slice is complete.
