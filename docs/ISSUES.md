...
- **Attempts/fixes:** v1.0.12–v1.0.19 hardened provider, viewport, storage, geometry and update seams. v1.0.20 commits through `8d394f289e02682bf27a133ed3456c7dc2852df9` added the tested Core `StageCGeneration` contract. This run then added `8bf703024753448e10dc77404f3ab7c946b3ce90`, `386956ed8ee2ee80a0e52082a7ad886ed8c048a8`, `3aec484d7e125e2e393b239562fb5c3c9f5a2da7`, `04fd660296e6d9657328de6b41b03c58fde334aa`, `dba4e4254717312e16b4aaf035d715b72c9b76cb`, and `cfd0352223cada181ea75843f16b25b9b5ceb541`: accepted source images are copied atomically into project-owned immutable ImageRevisions; the actual editor Accept-Baseline and Generate-3D callbacks are rewired onto a real ProjectSession; verified generated STL becomes a durable MeshRevision and Ready/Conflict candidate instead of immediate scene authority; Apply/Discard is explicit; applied candidates are restored visibly after restart.
- **Result:** the production baseline→3D candidate seam now uses the Stage-B Core contract while retaining legacy UI compatibility and existing AI job/cancellation ownership. The whole cleanup/export slice and real GTX1080 qualification remain incomplete.
- **Verification:** build run `34490666005` for `04fd660...` passed editor/launcher/updater/Core C#, Stage-B tests, Python core/job tests, real geometry regressions, release audit, packaging/hash and installer-definition checks. Later applied-object restore commits triggered fresh validation and must be reconciled before release.
- **Next action:** propagate stable generation identity through AIClient/backend job context, harden save-failure rollback, continue cleanup/export integration, then run the complete slice on the target machine.
...
- **Next action:** keep migrating the Stage-C vertical slice through job transport, cleanup and export before broadening to unrelated legacy subsystems.
...
- **Attempts/fixes:** cancellation reset in v1.0.13; job progress in v1.0.17/v1.0.18; v1.0.20 `StageCGeneration` adds stable `GenerationJobId`/binding carrying project identity, project revision, exact input ImageRevision and reserved output ObjectId. This run integrates that binding into the production editor submission/result-registration lifecycle and prevents stale baseline results from becoming authoritative. The existing `AIClient.Generate3DRoutedAsync` transport still sends path/prompt/output/provider/role only, so the stable binding is not yet carried through the HTTP/backend job record itself.
- **Result:** stale-result protection now guards the production editor side of the first vertical slice, but transport correlation, durable queue/resource ownership and recovery are still incomplete.
- **Next action:** extend the existing AIClient/backend path—without bypassing its job gate or cancellation ownership—to carry GenerationJobId, ProjectId, input project revision, ImageRevisionId and OutputObjectId through backend job context and validate/echo them before candidate registration.
